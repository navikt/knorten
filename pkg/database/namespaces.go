...
func (r *Repo) NamespaceUpdate(ctx context.Context, chartType gensql.ChartType, chartValues map[string]string, namespace string) error {
	tx, err := r.db.Begin()
	if err != nil {
		return err
	}
	...
	for key, value := range chartValues {
		...
		if err != nil {
			if err := tx.Rollback(); err != nil {
				r.log.WithError(err).Error("rolling back namespace update transaction - team chart value insert")
			}
			return err
		}
	}
